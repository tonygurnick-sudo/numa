import { CloudWatchLogsClient, GetLogEventsCommand, FilterLogEventsCommand, type OutputLogEvent, type FilteredLogEvent } from '@aws-sdk/client-cloudwatch-logs'
...
export interface LogTarget { group: string; stream?: string }
...
export function tailLogStream(opts: TailOptions) {
  ...
  async function tick() {
    if (stopped) return
    try {
      console.debug('[logs] tick', { group: opts.group, stream: opts.stream, started, startAtEnd: opts.startAtEnd, intervalMs: interval })
      if (!started) {
        // Prime nextToken by calling GetLogEvents; when startFromHead=false this positions us at the tail
        const prime = await cw.send(new GetLogEventsCommand({
        ...
        if (newToken && newToken !== nextToken) {
          ...
          if (idleTicks >= 6) {
            try {
              ...
              const filter = await cw.send(new FilterLogEventsCommand({
                logGroupName: opts.group,
              ...
              const events: OutputLogEvent[] = (filter.events ?? []).map((e: FilteredLogEvent) => ({ message: e.message, timestamp: e.timestamp }))
              ...
              const rePrime = await cw.send(new GetLogEventsCommand({
                logGroupName: opts.group,
                logStreamName: opts.stream,
                startFromHead: false,
                limit: 1,
  ...
  function start() {
    if (timer) return
    stopped = false
    console.debug('[logs] start tail', { group: opts.group, stream: opts.stream, startAtEnd: opts.startAtEnd, intervalMs: interval })
  ...
  function getLastTs() { return lastTs }
