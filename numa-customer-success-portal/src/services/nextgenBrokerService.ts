...
async function invoke<TReq, TRes = any>(payload: TReq): Promise<TRes> {
  ...
  const res = await lambda.send(new InvokeCommand({ FunctionName, Payload: new TextEncoder().encode(JSON.stringify(payload)) }))
...
export const nextgenBrokerService = {
  ...
  async getSystemUserSecret(accountId: string, secretName?: string, region?: string, roleArn?: string): Promise<{ secretName: string; secretString?: string }> {
