...
import { DynamoDBDocumentClient, QueryCommand, GetCommand, UpdateCommand, ScanCommand, DeleteCommand, PutCommand } from '@aws-sdk/lib-dynamodb'
...
const AUTO_GROUP_EXTRA_CLIENTS = new Set([
  'arcanum-prod-trial',
  'arcanum-prod-numa-demo',
  'arcanum-council-trial',
])
...
export async function startDeployment(input: StartDeploymentInput): Promise<{ executionArn: string; deploymentId: string }> {
...
export async function startGroupDeployment(input: StartGroupDeploymentInput): Promise<StartGroupDeploymentResult> {
  ...
  if (!input.clients || input.clients.length === 0) throw new Error('At least one client is required to start a group deployment')

  const credentials = getAwsCredentialsProvider()
  const region = getRegion()
  const sfn = new SFNClient({ region, credentials })

  const groupRunId = input.groupRunId || (typeof crypto !== 'undefined' && 'randomUUID' in crypto
...
export async function listDeploymentsByTimeRange(params: ListByTimeRangeParams): Promise<ListByTimeRangeResult> {
  ...
  if (items.length === 0 && !res.LastEvaluatedKey) {
    const scan = await ddb.send(new ScanCommand({
      TableName: table,
      FilterExpression: 'attribute_not_exists(historyPk) AND NOT begins_with(deploymentId, :lockPrefix) AND #s BETWEEN :from AND :to',
...
export async function getGroupDeploymentSummary(groupRunId: string, consistent = true): Promise<DeploymentRecord | null> {
...
export async function stopDeployment(deployment: DeploymentRecord, stoppedBy: string): Promise<void> {
  ...
  if (deployment.sfnExecutionArn) {
    try { await stopSfnExecution(deployment.sfnExecutionArn, `Stopped by ${stoppedBy}`) } catch (e) {
  ...
  if (deployment.ecsTaskArn) {
    try { await stopEcsTask(deployment.ecsTaskArn) } catch (e) {
  ...
  if (deployment.clientName && !deployment.clientName.startsWith('group#')) {
    try { await releaseDeploymentLock(deployment.clientName) } catch (e) {
