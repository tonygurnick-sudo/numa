...
export function ActivityFeed({
  ...
  const getActivityIcon = (type: string) => {
    switch (type) {
      ...
      case 'config': return <FileEarmark size={16} />
      case 'user': return <Person size={16} />
      case 'system': return <ExclamationTriangle size={16} />
          ...
          >
            ...
            <div className="flex-grow-1 min-w-0">
              ...
              <div className="text-muted small mt-1">
                {activity.subtitle}
                {activity.user && (
                  <span className="ms-2">• by {activity.user}</span>
                )}
              </div>
            </div>
          </div>
