...
export function ClientSelectGroup({
  ...
  const renderClientOption = (client: Client) => {
    const count = showCounts && deploymentCounts[client.name] !== undefined
      ? ` (${deploymentCounts[client.name]})`
      : ''
    return (
  ...
  return (
    ...
    >
      ...
      {devClients.length > 0 && (
        <>
          <optgroup label="Dev/Demo Stacks">
            {devClients.map(renderClientOption)}
          </optgroup>
        </>
      )}
      ...
      {productionClients.length > 0 && (
        <>
          <optgroup label="Client Stacks">
            {productionClients.map(renderClientOption)}
          </optgroup>
        </>
      )}
    </Form.Select>
