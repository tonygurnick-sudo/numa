...
export default function ConfigSearchTool() {
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['Client Name', 'Is Dev', 'Region', 'Account ID', 'Matched Fields', 'Apps', 'Agents', 'Integrations', 'All Prod Apps', 'Preferred KB']
    const rows = searchResults.map(r => [
      r.clientName,
      r.isDev ? 'Yes' : 'No',
      r.config.region || '',
      r.accountId || '',
      (r.matchDetails.length > 0 ? r.matchDetails : r.matchedFields.map(label => ({ label } as MatchDetail))).map(md => md.label).join('; '),
  ...
  const hasActiveSearch = Boolean(
    searchQuery.trim() || activeFilters.size > 0 || searchResults.length > 0
  )
      ...
      <div className="d-flex align-items-center mb-4">
        ...
        <div>
          ...
          <p className="text-muted mb-0">
            Search client configurations by keyword or filter by feature flags
          </p>
        </div>
      </div>
            ...
            <Card.Body>
              ...
              <Form.Group className="mb-4">
                ...
                <Form.Text className="text-muted">
                  Use <code>field=value</code> for match (strings are partial), or <code>field==value</code> for exact. Field names are case-insensitive. Free text searches the full config JSON (keys + values). Prefix with <code>!</code> to exclude (e.g., <code>!agents==true</code> or <code>!production</code>). Use <code>apps=name</code> to match app keys. Separate multiple terms with commas or new lines (AND logic).
                </Form.Text>
              </Form.Group>
                ...
                <small className="text-muted">
                  {loading ? (
                    ...
                    <>
                      ...
                      {hasActiveSearch && (
                        <> &bull; {searchResults.length} matching</>
                      )}
                    </>
                  )}
                </small>
                ...
                <div className="table-responsive" style={{ maxHeight: '70vh' }}>
                  <div className="d-flex flex-column gap-3">
                    {searchResults.map(result => {
                      ...
                      const matchDetails = result.matchDetails.length > 0
                        ? result.matchDetails
                          ...
                          <Card.Header className="d-flex justify-content-between align-items-center">
                            <div>
                              <div className="fw-semibold d-flex align-items-center gap-2">
                                ...
                                {result.isDev && (
                                  <Badge bg="warning" text="dark">Dev</Badge>
                                )}
                              </div>
                              ...
                              {matchDetails.map((detail, idx) => (
                                <div key={idx} className="d-flex align-items-center gap-2">
                                  ...
                                  <div>
                                    ...
                                    {detail.value && (
                                      <small className="text-muted">{detail.value}</small>
                                    )}
                                  </div>
                                </div>
                              ))}
                              ...
                              <Card.Footer className="bg-light">
                                <Row className="g-3">
                                  <Col md={4}>
                                    ...
                                    <div className="small">
                                      <div><strong>Client:</strong> {result.clientName}</div>
                                      <div><strong>Account ID:</strong> {result.accountId || 'n/a'}</div>
                                      <div><strong>Region:</strong> {result.config.region || 'n/a'}</div>
                                      <div><strong>Domain:</strong> {result.config.customDomain || 'n/a'}</div>
                                      <div><strong>Preferred KB:</strong> {result.config.preferredKnowledgeBase || 'bedrock'}</div>
                                      <div><strong>Integrations:</strong> {result.config.pipedreamIntegrations ? 'Enabled' : 'Disabled'}</div>
                                      <div><strong>Agents:</strong> {result.config.agents ? 'Enabled' : 'Disabled'}</div>
                                      <div><strong>All Prod Apps:</strong> {result.config.allProdApps ? 'Yes' : 'No'}</div>
                                    </div>
                                  </Col>
                                  <Col md={8}>
                                    <div className="fw-semibold mb-2">Configuration JSON</div>
                                    <pre className="bg-dark text-white p-3 rounded" style={{ maxHeight: '40vh', overflow: 'auto' }}>{JSON.stringify(result.config, null, 2)}</pre>
                                  </Col>
                                </Row>
                              </Card.Footer>
                            </div>
