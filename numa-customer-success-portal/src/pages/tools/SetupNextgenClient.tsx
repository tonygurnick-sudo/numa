    ...
    <Card className="border-0 shadow-sm">
      ...
      <Card.Body>
        ...
        <div className="mb-4">
          ...
          <div className="d-flex align-items-center gap-2">
            <Button onClick={onRename} disabled={!canRename || renaming}>
              {renaming ? (<><Spinner size="sm" className="me-2"/>Renaming…</>) : 'Rename Account'}
            </Button>
            {renameOk && <span className="text-success">Renamed to <strong>{renameOk.name}</strong></span>}
          </div>
          {renameErr && <Alert variant="danger" className="mt-2 mb-0">{renameErr}</Alert>}
        </div>

        {/* Step B: Client config (portal tool) */}
        <div className="mb-4">
          <h6 className="fw-semibold">Step B: Create/Update Client Config</h6>
          <p className="text-muted small mb-2">Use the portal tool to create or update the client configuration (supports JSON upload/replace).</p>
        ...
        <div className="mb-4">
          ...
          <p className="text-muted small mb-2">Use the Deployments page to run deploy by using Start Deployment. View live logs and manage locks.</p>
          <Button
            variant="primary"
        ...
        <div className="mb-1">
          ...
          <div className="d-flex align-items-center gap-2 flex-wrap">
            <Button variant="primary" onClick={onGetSecret} disabled={!accountId || gettingSecret}>
              {gettingSecret ? (<><Spinner size="sm" className="me-2"/>Fetching…</>) : 'Retrieve Secret'}
            </Button>
          ...
          {secretErr && <Alert variant="danger" className="mt-2 mb-0">{secretErr}</Alert>}
        </div>

        {/* Client URL */}
        <div className="mt-4">
          <h6 className="fw-semibold">Client Portal URL</h6>
          <p className="text-muted small mb-2">Quick link to the deployed client portal (constructed from Client ID).</p>
          {clientId.trim() ? (
            <a
              href={`https://${clientId.trim()}.numa.arcanum.ai`}
              target="_blank"
              rel="noreferrer"
            >
