...
export default function GetSystemUserSecret() {
  ...
  const onFetch = async () => {
    ...
    try {
      setFetching(true)
      const res = await nextgenBrokerService.getSystemUserSecret(accountId.trim(), secretName.trim() || undefined, region)
        ...
        <Row className="mb-3">
          ...
          <Col md={6}>
            <Form.Group>
              ...
              <Form.Control value={secretName} onChange={e => onSecretNameChange(e.target.value)} placeholder="e.g. numa-system-user-password" />
