...
export default function Deployments() {
  ...
  const [groupForm, setGroupForm] = useState<{ groupName: string; clients: string[]; description: string; maxConcurrency?: number }>({
    groupName: '',
    clients: [],
    description: '',
  ...
  const loadMoreHistory = useCallback(async () => {
    ...
    try {
      const fromIso = computeFromIso()
      const page: ListByTimeRangeResult = await listDeploymentsByTimeRange({ fromIso, exclusiveStartKey: historyNextKey, pageSize: 100 })
  ...
  const selectedGroup = useMemo(() => groups.find(group => group.groupName === selectedGroupName) ?? null, [groups, selectedGroupName])
  ...
  const canSingleDeploy = useMemo(
    () => Boolean(selectedClient && selectedTag && !isSingleSubmitting && sfnArn && !hasInFlight && deploymentLabel.trim().length > 0),
  ...
  const renderGroupRow = (record: DeploymentRecord) => {
    ...
    else if (statusLower === 'running' || statusLower === 'retrying' || statusLower === 'partial') statusVariant = 'warning'
        ...
        <tr className="align-middle table-light">
          ...
          <td className="py-3">
            <div className="d-flex align-items-center gap-2">
              <Badge bg="info" text="dark">Group</Badge>
          ...
          <td className="py-3">
            <div className="d-flex flex-column">
              ...
              {summaryTotal > 0 && (
                <small className="text-muted">{summaryCompleted}/{summaryTotal} complete</small>
              )}
            </div>
          </td>
          <td className="py-3">
          ...
          <td className="py-3">
            <div className="d-flex flex-wrap gap-2">
              ...
              <Button
                size="sm"
                variant="outline-secondary"
                onClick={() => toggleExpanded(record.deploymentId)}
              >
        ...
        {isExpanded && (
          <tr>
            <td colSpan={8} className="bg-light">
              <div className="p-3">
                <div className="mb-2"><strong>Group Run ID:</strong> {record.deploymentId}</div>
                {record.sfnExecutionArn && (
                  <div className="mb-2"><strong>State Machine Execution:</strong> {record.sfnExecutionArn}</div>
                )}
  ...
  const handlePlanOnly = async () => {
    ...
    try {
      ...
      await loadHistory()
      setTimeout(async () => { await loadHistory() }, 5000)
    } catch (e) {
  ...
  const handleReleaseLock = async (lock: DeploymentLockRecord) => {
    if (!confirm(`Release lock for ${lock.clientName}?\n\nOnly do this if the Step Function and ECS task have fully stopped.`)) return
    ...
    <div>
            ...
            <>
              {groupConfigMissing && (
                <Alert variant="info" className="mb-3">
                  Group deployments are not fully configured for this environment. Ensure <code>DEPLOYMENT_GROUP_SFN_ARN</code> and <code>DEPLOYMENT_GROUPS_TABLE</code> are present in the portal config.
                </Alert>
              )}
              {groups.length === 0 ? (
                  ...
                  <Col md={3}>
                    <Form.Group className="mb-3">
                      ...
                      <div className="form-text">
                        Up to {absoluteMaxGroupConcurrency} tasks in parallel{selectedGroup?.maxConcurrency ? ` (group default ${selectedGroup.maxConcurrency})` : ` (default ${defaultGroupConcurrency})`}.
                      </div>
                    </Form.Group>
                  </Col>
                  <Col md={4}>
              ...
              {selectedGroup && (
                <>
                  {selectedGroup.managed && (
                    <Alert variant="info" className="mb-3">
                      This group updates automatically to include every non-development client stack plus <code>arcanum-prod-trial</code>, <code>arcanum-prod-numa-demo</code>, and <code>arcanum-council-trial</code>.
                    </Alert>
                  )}
                  <div className="mb-3">
              ...
              <div className="d-flex align-items-center gap-2">
                <Button
                  ...
                  disabled={!canGroupDeploy || groupConfigMissing}
                >
                  {isGroupSubmitting ? (
                    <>
                      <Spinner size="sm" className="me-2" />
      ...
      <Card className="mt-4">
        ...
        <Card.Body className="p-0">
          {!deploymentsTable ? (
            <div className="p-3 text-muted">Deployment history table not configured. Missing <code>DEPLOYMENTS_TABLE</code> in config.</div>
          ) : (
            <div className="p-3 pb-0">
              ...
              <Tab eventKey="history" title="History">
                <div className="d-flex flex-wrap gap-2 justify-content-between align-items-center mb-3">
                  <ClientSelectGroup
                    ...
                    clients={clients}
                    showCounts={true}
                    deploymentCounts={clients.reduce((acc, c) => {
                  ...
                  />
                  <div className="d-flex align-items-center gap-2">
                    <Form.Select size="sm" value={historyPeriod} onChange={e => setHistoryPeriod(e.target.value as any)} style={{ width: 140 }}>
                ...
                <tbody>
                  {filteredHistory.length === 0 ? (
                    <tr>
                      <td colSpan={8} className="text-center py-4 text-muted">No deployments yet.</td>
                    </tr>
                  ) : (
                    filteredHistory.map(d => {
                      ...
                      const isStale = isRunning && d.startedAt ? (Date.now() - Date.parse(d.startedAt)) > 2 * 60 * 60 * 1000 : false
                      ...
                      if (isChildOfGroup) {
                        const newest = filteredHistory.find(x => x.deploymentId.includes('#') && x.groupRunId === d.groupRunId && x.clientName === d.clientName)
                      ...
                      return (
                        <Fragment key={d.deploymentId}>
                          <tr className={`align-middle${isSuperseded ? ' text-muted' : ''}`}>
                                ...
                                <div className="me-2">
                                  <div
                                    ...
                                    style={{
                                      width: '8px',
                                      height: '8px',
                                      backgroundColor: d.clientName.includes('demo') || d.clientName.includes('dev') ? '#ffc107' : '#28a745'
                                    }}
                                  ></div>
                                </div>
                            ...
                            <td className="py-3">
                              {d.deploymentLabel ? (
                                <span className="text-truncate" style={{ maxWidth: '220px', display: 'inline-block' }}>{d.deploymentLabel}</span>
                              ) : (
                            ...
                            <td className="py-3">
                              ...
                              {isStale && (
                                <div className="small text-danger">Stuck?</div>
                              )}
                            </td>
                            <td className="py-3">
                              <Badge bg={statusVariant}>{statusWithAttempt || 'unknown'}</Badge>
                              {isSuperseded && (
                                <span className="ms-2 small text-muted">retried</span>
                              )}
                            </td>
                            <td className="py-3">
                              <div className="d-flex flex-wrap gap-2">
                                ...
                                {isFailed && (
                                  <Button
                                    ...
                                    onClick={() => {
                                      void overrideDeploymentStatus(d.deploymentId, 'resolved')
                                        .then(loadHistory)
                                    }}
                                  >
                                    Mark Resolved
              ...
              {historyNextKey && (
                <div className="text-center py-3">
                  <Button variant="outline-secondary" size="sm" onClick={loadMoreHistory}>Load More</Button>
                </div>
              )}
              </Tab>
              <Tab
                ...
                title={(
                  <span>
                    ...
                    {groupRecords.length > 0 && (
                      <Badge bg="secondary" className="ms-1">{groupRecords.length}</Badge>
                    )}
                  </span>
                )}
              >
                ...
                <Table hover responsive className="mb-0">
                  ...
                  <tbody>
                    {groupRecords.length === 0 ? (
                      <tr>
                        <td colSpan={8} className="text-center py-4 text-muted">No group deployments yet.</td>
                      </tr>
                    ) : (
              ...
              <Tab eventKey="locks" title={`Locks${locks.length ? ` (${locks.length})` : ''}`}>
                <div className="mb-3">
                  <Alert variant="warning">
                    Deployment locks prevent concurrent runs per client. Only release a lock after confirming the Step Function and ECS task have finished.
                  </Alert>
                </div>
                <Table hover responsive className="mb-0">
                  <thead>
                    ...
                    {sortedLocks.map(lock => {
                      ...
                      return (
                        <tr key={lock.deploymentId} className="align-middle">
                          ...
                          <td className="py-3"><small className="text-muted">{formatStartedNz(lock.startedAt)}</small></td>
                          <td className="py-3"><code>{lock.deploymentId}</code></td>
                          <td className="py-3">
                            <Button
                              variant="outline-danger"
                              size="sm"
      ...
      <Modal show={showConfirmModal} onHide={handleCancelDeploy} centered>
        ...
        <Modal.Body>
          ...
          <div className="bg-light p-3 rounded">
            <div><strong>Client:</strong> {selectedClient}</div>
            ...
            {deploymentLabel && (
              <div><strong>Deployment Label:</strong> {deploymentLabel}</div>
            )}
          </div>
          <div className="mt-3">
      ...
      <Modal show={groupModalOpen} onHide={handleGroupModalClose} size="lg" centered>
        <Modal.Header closeButton>
          <Modal.Title>{editingGroup ? `Edit Group: ${editingGroup.groupName}` : 'Create Deployment Group'}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
            ...
            <Row className="gy-3">
              ...
              <Col md={6}>
                <Form.Group>
                  ...
                  <Form.Text>Leave blank to use the portal default ({defaultGroupConcurrency}). Hard limit {absoluteMaxGroupConcurrency}.</Form.Text>
                </Form.Group>
                {editingGroup && (
                  <ListGroup variant="flush" className="small mt-3">
                    ...
                    <ListGroup.Item className="px-0">Last updated: {editingGroup.updatedAt ? new Date(editingGroup.updatedAt).toLocaleString() : 'unknown'}</ListGroup.Item>
                  </ListGroup>
                )}
              </Col>
            </Row>
        ...
        <Modal.Footer>
          ...
          <Button variant="outline-secondary" onClick={handleGroupModalClose} disabled={groupModalBusy}>Cancel</Button>
