...
function QAppsManager({ temporaryCredentials }) {
  ...
  useEffect(() => {
    if (usernameRef.current) {
      usernameRef.current.value = 'testuser@arcanum.ai';
    }
  }, []);
  ...
  const login = async (username, password, setSessionToken) => {
    ...
    const API_ENDPOINT =
      'https://g59jhyyob7.execute-api.us-east-1.amazonaws.com';
  ...
  const initializeQAppsClient = async () => {
    ...
    try {
      ...
      const [poolId, roleArn] = await Promise.all([
      ...
      const credentials = fromWebToken(webTokenBody);
  ...
  const fetchLibraryItems = async () => {
    ...
    try {
      ...
      setLibraryItems(response.libraryItems || []);
    } catch (error) {
