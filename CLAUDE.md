...
- **Self-optimisation:** Numa is designed to compound — get cheaper and more reliable the more a user uses it. Any surface that runs an agent (scheduled runs, agent types, app runners) should carry reflect-and-compound prompting: notice deterministic work and script it as a saved workflow, notice durable facts and save them as a memory — but **script mechanics, never judgment** (agents must stay able to think). See `documentation/self-optimising-numa.md`.
...
| `documentation/self-optimising-numa.md` | Self-Optimising Numa (FEAT-243): the compounding pattern — scheduled agents reflect and script deterministic steps / save memories; chat actively saves workflows + memories; agent-scoped workflow library; cost feedback; "script mechanics, never judgment". Read before adding any agent execution surface. |
