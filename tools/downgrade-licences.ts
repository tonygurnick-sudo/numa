...
async function sign(credentials, method, url: URL, service: string, region?: string, body?: string): Promise<Request> {
  region ??= 'us-east-1';
...
async function listSubscriptions(credentials, applicationId: string): Promise<Array<Subscription>> {
  ...
  do {
    ...
    const response = await (await fetch(await sign(credentials, 'GET', url, 'qbusiness'))).json();
...
async function setSubscription(credentials, applicationId: string, subscriptionId: string): Promise<boolean> {
  const url = new URL(
    `https://qbusiness.us-east-1.api.aws/applications/${applicationId}/subscriptions/${subscriptionId}`,
  );
  const res = await fetch(
    await sign(credentials, 'PUT', url, 'qbusiness', 'us-east-1', JSON.stringify({ type: desiredSubscription })),
  );
  // console.log(await res.text());
  return res.status == 200;
}

async function deleteSubscription(credentials, applicationId: string, subscriptionId: string): Promise<boolean> {
  console.log('deleting...');
  const url = new URL(
    `https://qbusiness.us-east-1.api.aws/applications/${applicationId}/subscriptions/${subscriptionId}`,
  );
  const res = await fetch(await sign(credentials, 'DELETE', url, 'qbusiness', 'us-east-1'));
  return res.status == 200;
}
...
if (import.meta.filename === process?.argv[1]) {
  ...
  const subscriptions = await listSubscriptions(credentials, accountDetails.qApplicationId);
  ...
  for (const sub of subscriptions) {
    ...
    if (!username) {
      await deleteSubscription(credentials, accountDetails.qApplicationId, sub.subscriptionId);
    } else if (sub.nextSubscription.type === desiredSubscription) {
      continue;
    } else if (!excludedUsernames.includes(username)) {
      await setSubscription(credentials, accountDetails.qApplicationId, sub.subscriptionId);
