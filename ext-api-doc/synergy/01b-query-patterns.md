...
File search (content search supported via `Contents`):
`POST /api/v1/files/search` `{"FileName":"drainage","Contents":"","Page":1,"PageSize":50,"ShowDeletedFiles":false,"RetrieveAttributes":true,"LimitSearchTo":0,"LimitID":{"IDString":"100_1"}}`
`LimitSearchTo`: 0=all, 1=job, 2=folder (verify per API); `LimitID` only needed if `LimitSearchTo>0`.
...
Ex 5 — Full-text search inside files: `POST /api/v1/files/search` `{"FileName":"","Contents":"drainage","Page":1,"PageSize":20,"ShowDeletedFiles":false,"RetrieveAttributes":true}`
