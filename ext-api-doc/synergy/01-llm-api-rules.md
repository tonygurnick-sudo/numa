...
- "Search inside file contents for 'drainage'": `search-files synergy drainage` → backend `POST /api/v1/files/search` `{Contents:"drainage",Page:1,PageSize:20}`; narrow with `LimitSearchTo`+`LimitID`.
