...
Strategy 1 — modified-date (preferred). For entities with date fields (`LastModified`, `CreatedDate`), poll and filter client-side:

```python
last_poll = "2026-03-28T00:00:00Z"
page = 1
while True:
    data = POST("/api/v1/files/search", json={"FileName":"","Contents":"","Page":page,"PageSize":100,"ShowDeletedFiles":False,"RetrieveAttributes":True}).json()
...
Applies to Files (LastModified), Jobs (CreatedDate), etc. Limitation: must page through all results; server-side date filtering not guaranteed (verify search support per entity).

Strategy 2 — search-based (server-side date filter, if supported). `Attributes` array on `JobSearchModel`/`FileSearchModel`:

```
POST /api/v1/files/search
{"FileName":"","Page":1,"PageSize":100,"Attributes":[{"Attribute":{"Name":"ModifiedDate"},"Value":"2026-03-28T00:00:00Z","SearchQueryType":4,"Operation":2,"OperationName":">"}]}
```
