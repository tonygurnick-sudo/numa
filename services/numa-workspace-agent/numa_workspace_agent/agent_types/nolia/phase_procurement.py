"""Phase 3a: Procurement-Specific Rules agent type."""
...
from .prompts.procurement_rules import NOLIA_PROCUREMENT_RULES_ADDENDUM


def build_nolia_procurement_prompt(**kwargs) -> str:
    """Build system prompt for the Nolia Procurement Rules phase."""
    kwargs["identity_override"] = NOLIA_SPECIALIST_IDENTITY
    base = build_nolia_system_prompt(**kwargs)
    return base + NOLIA_PROCUREMENT_RULES_ADDENDUM


NOLIA_PROCUREMENT = AgentTypeConfig(
    type_id="nolia-procurement",
    display_name="Nolia Procurement Rules (Phase 3a)",
    response_mode="sync",
    system_prompt_builder=build_nolia_procurement_prompt,
...
register_agent_type(NOLIA_PROCUREMENT)
