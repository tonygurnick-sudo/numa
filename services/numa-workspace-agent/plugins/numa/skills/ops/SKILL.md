...
10. **Read ticket bodies before clustering or triaging.** Title-only grouping miscategorises tickets and the error cascades — one bench bucketed a ticket from its title alone, then produced wrong parent-bug links and off-voice drafts for six turns after. When grouping, linking, or summarising, read the descriptions (`get_ticket`), not just the titles.
11. **Filter explicitly on status and date.** When the user asks for "open tickets from the last 10 days", apply BOTH filters in the query — don't pull everything and eyeball it, or closed and out-of-window tickets leak in and get reported as current. State the filters you applied.
12. **Re-query before confirming something exists.** When asked to verify that tickets/links were created ("confirm they're all there"), re-query the source — don't answer from memory of what you did earlier. One bench "confirmed" 11 artifacts with zero tool calls; they weren't all there. Verification means a fresh read.
