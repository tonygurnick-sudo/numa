...
**Downloads:** Use `"filePath": "/tmp/filename.ext"` in props with `stash_id="NEW"`. File appears in `/workdir/tmp/integrations-results/` (scratch — hidden from the user's Files page). If the user asked for the file as a deliverable, `cp` it to `/workdir/outputs/` after downloading.
...
4. **Read large results** from `/workdir/tmp/integrations-results/` (auto-saved); promote to `/workdir/outputs/` only if the user wants the file as a deliverable
