...
<Note>
  See [Setting the polling interval](/connect/components/triggers#setting-the-polling-interval) for details on defining the schedule for polling-based triggers.
</Note>

## OpenAPI

```yaml post /v1/connect/{project_id}/triggers/deploy
openapi: 3.0.4
info:
  version: 2.0.0
...
components:
  schemas:
    ...
    DeployTriggerOpts:
      type: object
      description: Request options for deploying a trigger
      required:
        - id
        - external_user_id
      properties:
        id:
          type: string
          description: The trigger component ID
        version:
          type: string
          description: >-
            Optional trigger component version (in SemVer format, for example
        ...
        workflow_id:
          ...
          description: Optional ID of a workflow to receive trigger events
        webhook_url:
          type: string
          description: Optional webhook URL to receive trigger events
        emit_on_deploy:
          type: boolean
          description: >-
            Whether the trigger should emit events during the deploy hook
            execution. Defaults to true if not specified.
          nullable: true
    DeployTriggerResponse:
      type: object
      description: Response received after deploying a trigger
      required:
        - data
      properties:
        data:
