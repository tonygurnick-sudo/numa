...
**Native-connector file browsing.** Native connectors expose two surfaces through `oauth_workspace_tools`: authenticated HTTP via `integrations request` (`connect_request`), and a browsable file tree via `integrations list-files` / `search-files` / `download-file` / `file-info`. The file commands route per connector — Synergy 12d → `connect_synergy_{list,search,download}` (jobs modelled as folders, `job:` / `folder:` id prefixes), the OAuth cloud-storage providers (Google Drive, Gmail, OneDrive, Dropbox) → `oauth_{list_files,search_files,download_file,get_file_metadata}` keyed by a `provider` param. **Synergy is a file-store connector and does NOT support `request`** — browse it with the file commands. These file ops were restored after the MCP→CLI migration (commit `6bebe5603`) dropped the legacy MCP `connect.py` file tools without porting them — the backend handlers always existed, they just needed `registry.ts` routes (→ `oauth_workspace_tools`) + the CLI commands. The file-browse allow-list lives in `commands/actions/integrations.ts` (`FILE_BROWSE_ROUTING`) and mirrors the connector registry's `surfaces: ['files']` set — keep them in sync when adding a file-browsing connector. `registry.test.ts` pins the routes so the drop can't silently recur.
