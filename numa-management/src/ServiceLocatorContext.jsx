...
export function ServiceLocatorProvider({ children }) {
  ...
  const fetchSystemUserPassword = async (temporaryCredentials, profileName) => {
    try {
      if (!temporaryCredentials) {
        console.log('Waiting for temporary credentials...');
        return null;
      ...
      const valueCommand = new GetSecretValueCommand({
        SecretId: profileName + '-system-user-password',
      });

      return JSON.parse((await secretsClient.send(valueCommand)).SecretString).password;
    } catch (error) {
      console.error('Error fetching system user password:', error);
      setError('Failed to fetch system user password: ' + error.message);
      return null;
    }
  };
