...
- When searching emails:
    - Set Metadata Only = false to fetch full payload/parts, required for attachments discovery (required to discover parts[].body.attachmentId).
    - Set Return payload as plaintext = false if you need original HTML or to parse MIME parts for attachments.
    - Use Gmail query syntax in Search Query (e.g., from:alice has:attachment newer_than:7d).
    - Include Labels and Include Spam and Trash only when needed to widen the search.
Tips:
