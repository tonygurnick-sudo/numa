...
try:
    ...
    from template_engine import TemplateManager, TemplateValidator
except ImportError as e:
    # Fallback for development/testing
    logger = structlog.get_logger()
    logger.warning("Could not import consolidated storage modules", error=str(e))

    def get_consolidated_vault(*args, **kwargs):
        return {"secrets": {}}

    def get_vault_secret(*args, **kwargs):
        return None

    def add_secret_to_vault(*args, **kwargs):
        return {}

    def remove_secret_from_vault(*args, **kwargs):
        return False

    def create_secret_from_template(*args, **kwargs):
        return {}
    ...
    class TemplateManager:
        ...
        def track_template_usage(self, *args):
            pass
...
def get_all_user_oauth_connectors(user_sub: str) -> Dict[str, List[Dict[str, Any]]]:
    ...
    try:
        ...
        for secret_name, secret_data in user_vault.get("secrets", {}).items():
            ...
            elif any(p in secret_name.lower() for p in FRIENDLY_NAMES.keys()):
                # Try to match provider from secret name
                for p in FRIENDLY_NAMES.keys():
        ...
        for provider in connectors_by_provider:
            connectors_by_provider[provider].sort(
...
def create_custom_oauth_connector(
    ...
    try:
        ...
        from template_engine import FreeFormValidator
