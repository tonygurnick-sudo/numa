...
def generate_stats_content_html(
  ...
  <div style="margin-top:40px;padding-top:20px;border-top:1px solid #E5E7EB;font-size:0.9em;color:#6B7280;text-align:center;">
    <p>Generated by Beyond Expectations Error Monitoring System on {datetime.datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S UTC")}</p>
        ...
        if grouped:
            ...
            for client_id, notifs in grouped.items():
                ...
                for n in notifs:
                    ...
                    occ = log.get("occurrences", 1)
                    first_occ = log.get("first_occurrence", "Unknown")
                    last_occ = log.get("last_occurrence", "Unknown")
                    mail_to = ""
                    for link in mail_to_links:
                        if (
                            ...
                            break
                    html.append(
                        f"<div class='error-item'><h3><span class='severity-badge {badge}'>{sev}</span>{err}</h3><div class='client-info'><strong>Task:</strong> {task}<br><strong>Occurrences:</strong> {occ}<br><strong>First Occurrence:</strong> {first_occ}<br><strong>Last Occurrence:</strong> {last_occ}</div><p><strong>Explanation:</strong> {expl}</p><p><strong>Recommended Action:</strong> {actn}</p><div class='error-message'>{msg}</div>{('<a href=\"'+mail_to+'\" class=\"mail-to-button\">Compose Email to Client</a>') if mail_to else ''}</div>"
                    )
                html.append("</div></div>")
        else:
            html.append(
