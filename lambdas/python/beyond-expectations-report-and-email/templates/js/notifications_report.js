document.addEventListener('DOMContentLoaded', function () {
  showTopTab('top-new');
  showTab('new-tab1');
  ['new-tab1', 'new-tab2', 'new-tab3', 'new-tab4', 'rec-tab1', 'rec-tab2', 'rec-tab3', 'rec-tab4'].forEach(initFilters);
});

function showTab(id) {
  document.querySelectorAll('.tab-content:not(.top)').forEach((t) => (t.style.display = 'none'));
...
function showTopTab(id) {
  document.querySelectorAll('.tab-content.top').forEach((t) => (t.style.display = 'none'));
  document.getElementById(id).style.display = 'block';
  document.querySelectorAll('.tab-label.top').forEach((l) => l.classList.remove('active'));
  document.getElementById(id + '-label').classList.add('active');
...
function initFilters(tabId) {
  ...
  tab.querySelectorAll('.client-section-header div:first-child').forEach((d) => clients.add(d.textContent.trim()));
  ...
  // severity filter
  const sevs = new Set();
  tab.querySelectorAll('.severity-badge').forEach((b) => sevs.add(b.textContent.trim()));
...
function applyFilters(tabId) {
  ...
  tab.querySelectorAll('.client-section').forEach((sec) => {
    const name = sec.querySelector('.client-section-header div:first-child').textContent.trim();
    ...
    if (show && sevVal !== 'all') {
      ...
      sec.querySelectorAll('.error-item').forEach((item) => (item.style.display = ''));
    }
    sec.style.display = show ? '' : 'none';
  });
  const visible = Array.from(tab.querySelectorAll('.client-section')).some((s) => s.style.display !== 'none');
