...
const PUBSUB_TOPIC = process.env.PUBSUB_TOPIC as string;
 ...
 */
const getGmailAccessToken = async (userSub: string): Promise<string | null> => {
  ...
  try {
    ...
    return gmailEntry.fields.access_token;
  } catch (error: unknown) {
 ...
 */
const renewGmailWatch = async (accessToken: string): Promise<GmailWatchResponse> => {
...
export const handler: Handler = async () => {
  ...
  if (!DATA_CONNECTORS_TABLE_NAME || !CLIENT_NAME || !PUBSUB_TOPIC) {
  ...
  for (const connector of connectors) {
    ...
    try {
      // 2a. Get the access token from the user's vault
      const accessToken = await getGmailAccessToken(userId);
      if (!accessToken) {
        console.warn(`Skipping user ${userId}: no access token available`);
        failed++;
        continue;
      }

      // 2b. Call Gmail users.watch()
      const watchResponse = await renewGmailWatch(accessToken);
