...
export const POST_CALL_PROMPT = [
  ...
  'Then follow your system instructions: summarise, extract objections and next steps, rate the call, suggest follow-ups, and update the matched prospect record in master_prospects.json + today_calls.json. Set call_summary, call_date, status, and call_outcome (map {{ event.sdr_outcome }} to exactly one of: interested | callback | no_answer | not_interested). If a callback was agreed, also set callback_date.',
  'You are the SINGLE writer for this prospect record this call. Qualification promotion runs ONLY if qualified is explicitly the literal true — NOT false, and NOT null/empty. A null/absent qualified means the SDR wrap-up was never captured: in that case do NOT promote, and note in the summary that the wrap-up was missing so a human can follow up. When qualified is explicitly true and Numa Ops is available, create the Numa Ops CRM customer record (attach the summary + transcript, add a follow-up task, assign the AE), write crm_record_id back, and set qualified: true. Do all prospect-record edits in one read-modify-write so nothing is lost.',
].join('\n');
