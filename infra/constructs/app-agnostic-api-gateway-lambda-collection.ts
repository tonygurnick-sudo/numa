...
export class AppAgnosticApiGatewayLambdaCollection extends ApiGatewayLambdaCollection {
  ...
  constructor(scope: Construct, name: string, props: AppAgnosticApiGatewayLambdaCollectionProps) {
    ...
    // ── Synergy CONNECTOR routes (gated on synergyKbCrawl) ───────────────────
    // The crawl/index control plane: manual "Sync now" + admin crawl config +
    // run status. Only deploy when the Synergy connector flag is on — when off
    ...
    // ── Synergy REMOTE FILES BROWSER routes (gated on synergyFileParity) ──────
    ...
    if (props.synergyFileParityEnabled) {
...
> {
  ...
  /** Synergy CONNECTOR flag (synergyKbCrawl && dataConnectorsEnabled). Gates the
   *  crawl/index control-plane routes (sync-now, sync-config GET/PUT, sync-status)
   *  so they only deploy when the connector is enabled — mirrors the crawler
   *  construct, which is provisioned on the same flag. */
  synergyKbCrawlEnabled?: boolean;
  /** Synergy REMOTE FILES BROWSER flag (synergyFileParity && dataConnectorsEnabled).
   *  Gates the interactive browse + read-parity routes (jobs, job folders, folder
   *  items, file search/history/weblink/details) so the browser API surface only
   *  deploys when the browser is enabled. */
  synergyFileParityEnabled?: boolean;
