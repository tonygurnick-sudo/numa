...
describe('KnowledgeBaseConstruct', () => {
  ...
  it('Creates subnets', () => {
    ...
    assert(
      Testing.toHaveResourceWithProperties(synthesized, 'aws_subnet', {
        ...
        tags: {
          Name: 'test-knowledge-base-ap-southeast-2a',
        },
      }),
    );
    assert(
      Testing.toHaveResourceWithProperties(synthesized, 'aws_subnet', {
        ...
        tags: {
          Name: 'test-knowledge-base-ap-southeast-2b',
        },
      }),
    );
  });
  ...
  it('Creates a knowledge base', () => {
    assert(Testing.toHaveResourceWithProperties(synthesized, 'aws_iam_role', { name: 'test-knowledge-base' }));
    const roleId = Object.entries(stackObject.resource.aws_iam_role as Record<string, { name: string }>).filter(
      ([, value]) => value.name === 'test-knowledge-base',
    )[0][0];
    assert(Testing.toHaveResourceWithProperties(synthesized, 'aws_iam_policy', { name: 'test-knowledge-base' }));
    assert(
    ...
    ).filter(([, value]) => value.name === 'test-bedrock-user')[0][0];
    assert(
  ...
  it('Creates the bedrock user secret', () => {
    assert(
      Testing.toHaveResourceWithProperties(synthesized, 'aws_secretsmanager_secret', { name: 'test-bedrock-user' }),
    );
  });

  it('Creates the lambda function', () => {
    assert(Testing.toHaveResourceWithProperties(synthesized, 'aws_iam_role', { name: 'test-knowledge-base-init' }));
    const roleId = Object.entries(stackObject.resource.aws_iam_role as Record<string, { name: string }>).filter(
      ([, value]) => value.name === 'test-knowledge-base-init',
    )[0][0];
    assert(Testing.toHaveResourceWithProperties(synthesized, 'aws_iam_policy', { name: 'test-knowledge-base-init' }));
    ...
    assert(
      Testing.toHaveResourceWithProperties(synthesized, 'aws_lambda_function', {
        functionName: 'test-knowledge-base-init',
