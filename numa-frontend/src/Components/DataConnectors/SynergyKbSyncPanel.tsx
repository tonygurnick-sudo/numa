 ...
 * only where the crawler is deployed (DEPLOY_SYNERGY_KB_SEARCH). Lets an admin
...
export const SynergyKbSyncPanel = () => {
  ...
  const crawlerDeployed = sessionStorage.getItem('DEPLOY_SYNERGY_KB_SEARCH') === 'true' && getFlag('SYNERGY_KB_SEARCH');
        ...
        <>
          ...
          <div className="d-flex align-items-center gap-2 mb-3">
            <Button
              ...
              size="sm"
              disabled={saving || config?.credential_user_sub === user?.sub}
              onClick={() => void save({ use_my_credential: true })}
            >
              {config?.credential_user_sub === user?.sub
